import { useEffect } from 'react'
import Navbar from '../components/Navbar'
import Footer from '../components/Footer'
import './Contact.css'

const Contact = () => {
  useEffect(() => {
    window.scrollTo(0, 0)
  }, [])

  const contactMethods = [
    {
      iconType: 'email',
      label: 'Email',
      value: 'dhiren@example.com',
      link: 'mailto:dhiren@example.com',
    },
    {
      iconType: 'linkedin',
      label: 'LinkedIn',
      value: 'linkedin.com/in/dhirenpurasinghe',
      link: 'https://linkedin.com/in/dhirenpurasinghe',
    },
    {
      iconType: 'github',
      label: 'GitHub',
      value: 'github.com/dhirenpurasinghe',
      link: 'https://github.com/dhirenpurasinghe',
    },
    {
      iconType: 'twitter',
      label: 'Twitter',
      value: '@dhirenpurasinghe',
      link: 'https://twitter.com/dhirenpurasinghe',
    },
  ]

  const renderIcon = (iconType: string) => {
    switch (iconType) {
      case 'email':
        return (
          <svg viewBox="0 0 24 24" fill="currentColor" width="40" height="40">
            <path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/>
          </svg>
        )
      case 'linkedin':
        return (
          <svg viewBox="0 0 24 24" fill="currentColor" width="40" height="40">
            <path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z"/>
          </svg>
        )
      case 'github':
        return (
          <svg viewBox="0 0 24 24" fill="currentColor" width="40" height="40">
            <path d="M12 2A10 10 0 0 0 2 12c0 4.42 2.87 8.17 6.84 9.5.5.08.66-.23.66-.5v-1.69c-2.77.6-3.36-1.34-3.36-1.34-.46-1.16-1.11-1.47-1.11-1.47-.91-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.87 1.52 2.34 1.07 2.91.83.09-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.92 0-1.11.38-2 1.03-2.71-.1-.25-.45-1.29.1-2.64 0 0 .84-.27 2.75 1.02.79-.22 1.65-.33 2.5-.33.85 0 1.71.11 2.5.33 1.91-1.29 2.75-1.02 2.75-1.02.55 1.35.2 2.39.1 2.64.65.71 1.03 1.6 1.03 2.71 0 3.82-2.34 4.66-4.57 4.91.36.31.69.92.69 1.85V21c0 .27.16.59.67.5C19.14 20.16 22 16.42 22 12A10 10 0 0 0 12 2z"/>
          </svg>
        )
      case 'twitter':
        return (
          <svg viewBox="0 0 24 24" fill="currentColor" width="40" height="40">
            <path d="M22.46 6c-.77.35-1.6.58-2.46.69.88-.53 1.56-1.37 1.88-2.38-.83.5-1.75.85-2.72 1.05C18.37 4.5 17.26 4 16 4c-2.35 0-4.27 1.92-4.27 4.29 0 .34.04.67.11.98C8.28 9.09 5.11 7.38 3 4.79c-.37.63-.58 1.37-.58 2.15 0 1.49.75 2.81 1.91 3.56-.71 0-1.37-.2-1.95-.5v.03c0 2.08 1.48 3.82 3.44 4.21a4.22 4.22 0 0 1-1.93.07 4.28 4.28 0 0 0 4 2.98 8.521 8.521 0 0 1-5.33 1.84c-.34 0-.68-.02-1.02-.06C3.44 20.29 5.7 21 8.12 21 16 21 20.33 14.46 20.33 8.79c0-.19 0-.37-.01-.56.84-.6 1.56-1.36 2.14-2.23z"/>
          </svg>
        )
      default:
        return null
    }
  }

  const skills = [
    'React',
    'TypeScript',
    'Node.js',
    'Python',
    'Docker',
    'AWS',
    'MongoDB',
    'PostgreSQL',
    'GraphQL',
    'REST APIs',
    'Git',
    'Linux',
  ]

  return (
    <div className="contact-page">
      <Navbar />

      <div className="contact-page-container">
        <div className="contact-hero">
          <h1 className="contact-page-title">Get In Touch</h1>
          <p className="contact-page-subtitle">
            I'm always interested in hearing about new projects and opportunities. Let's connect!
          </p>
        </div>

        <div className="contact-grid">
          <div className="contact-main">
            <div className="contact-section">
              <h2 className="section-heading">Contact Information</h2>
              <div className="contact-methods">
                {contactMethods.map((method) => (
                  <a
                    key={method.label}
                    href={method.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="contact-method"
                  >
                    <span className="method-icon">{renderIcon(method.iconType)}</span>
                    <div className="method-info">
                      <span className="method-label">{method.label}</span>
                      <span className="method-value">{method.value}</span>
                    </div>
                    <span className="method-arrow">→</span>
                  </a>
                ))}
              </div>
            </div>

            <div className="contact-section">
              <h2 className="section-heading">About Me</h2>
              <div className="about-content">
                <p className="about-text">
                  I'm a multiskilled programmer passionate about building efficient, scalable, and
                  user-friendly applications. With experience across the full stack, I enjoy tackling
                  complex problems and learning new technologies.
                </p>
                <p className="about-text">
                  Currently open to freelance projects, full-time opportunities, and collaborative
                  ventures. I specialize in web development but am always eager to explore new domains.
                </p>
              </div>
            </div>

            <div className="contact-section">
              <h2 className="section-heading">Technical Skills</h2>
              <div className="skills-grid">
                {skills.map((skill) => (
                  <div key={skill} className="skill-item">
                    {skill}
                  </div>
                ))}
              </div>
            </div>
          </div>

          <div className="contact-sidebar">
            <div className="resume-card">
              <div className="resume-icon">📄</div>
              <h3 className="resume-title">Resume</h3>
              <p className="resume-description">
                Download my full resume to learn more about my experience and qualifications.
              </p>
              <button className="download-resume-btn">Download Resume (PDF)</button>
            </div>

            <div className="availability-card">
              <div className="status-indicator">
                <span className="status-dot"></span>
                <span className="status-text">Available for Work</span>
              </div>
              <p className="availability-text">
                I'm currently open to new opportunities and interesting projects. Feel free to reach out!
              </p>
            </div>
          </div>
        </div>
      </div>
      <Footer />
    </div>
  )
}

export default Contact
